...
describe("TwabLib", () => {
  ...
  beforeEach(async () => {
    ...
    cardinality = await twabLib.connect(wallet1).MAX_CARDINALITY();
  });
  ...
  describe("getAverageBalanceBetween()", () => {
    ...
    context("with two twabs", () => {
      ...
      */
      ...
      it("should return an average of zero for pre-history requests", async () => {
        expect(await twabLib.getAverageBalanceBetween(timestamp1 - 100, timestamp1 - 50, currentTime)).to.equal(
          toWei("0")
        );
      });

      it("should return half the minted balance when the duration is centered over first twab", async () => {
        expect(await twabLib.getAverageBalanceBetween(timestamp1 - 50, timestamp1 + 50, currentTime)).to.equal(
          toWei("500")
        );
      });

      it("should return an accurate average when the range is between twabs", async () => {
        expect(await twabLib.getAverageBalanceBetween(timestamp1 + 50, timestamp1 + 55, currentTime)).to.equal(
          toWei("1000")
        );
      });

      it("should return an accurate average when the end is after the last twab", async () => {
        expect(await twabLib.getAverageBalanceBetween(timestamp2 - 50, timestamp2 + 50, currentTime)).to.equal(
          toWei("750")
        );
      });

      it("should return an accurate average when the range is after twabs", async () => {
        expect(await twabLib.getAverageBalanceBetween(timestamp2 + 50, timestamp2 + 51, currentTime)).to.equal(
          toWei("500")
        );
      });
    });
  ...
  describe("getBalanceAt", () => {
    ...
    describe("with problematic query", () => {
      ...
      it("should work", async () => {
        expect(await twabLib.getBalanceAt(1630713395, parseInt("" + new Date().getTime() / 1000 + 1000))).to.equal(
          toWei("100")
        );
      });
    });
  ...
  describe("push()", () => {
    it("should increment the cardinality", async () => {
      ...
      expect(result.map((obj: any) => obj.toString())).to.deep.equals(["0", "3", `11`]);
    });
    ...
    it("should correctly limit the cardinality", async () => {
      ...
      expect(result.map((obj: any) => obj.toString())).to.deep.equals(["0", "3", `${2 ** 24 - 1}`]);
    });
  });
});
