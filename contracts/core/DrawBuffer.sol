 ...
 */
contract DrawBuffer is IDrawBuffer, Manageable, Constants {
    ...
    function setDraw(
        ...
        require(
            msg.sender == owner() || msg.sender == prizeDistributor,
            "DrawBuffer/caller-is-not-owner-nor-prize-distributor"
        );

        uint32 index = bufferMetadata.getIndex(_newDraw.drawId);
     ...
     * @dev    The getNewestDraw.drawId() is used to calculate a Draws ID delta
     *         position.
